...
def export():
    ...
    for post in posts:
        ...
        # SEO enrichment (intro/materials/hook/faq…), generated by /enrich → posts.meta_json
        meta_raw = post.get("meta_json")
        if meta_raw:
            try:
                meta = json.loads(meta_raw)
                if isinstance(meta, dict) and meta:
                    pattern_entry["meta"] = meta
            except (ValueError, TypeError):
                print(f"  [!] {slug}: invalid meta_json, skipped")

        patterns_data.append(pattern_entry)
        print(f"  [{slug}] {len(image_entries)} images, {len(patterns)} patterns"
              + (" +meta" if pattern_entry.get("meta") else ""))
